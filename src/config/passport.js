...
const jwt = async (payload, done) => {
	try {
		...
		if(moment(payload.exp).isBefore((new Date().getTime())/1000)) {
			...
			const { user, accessToken } = await User.findAndGenerateToken({email: userObj.email, refreshObject});
...
const googleAuth = async (req, accessToken, refreshToken, profile, done) => {
	try {
		if(req.user) {
			...
			const user = await User.oAuthLogin(profile.provider, profile.id, profile._json.email, profile.displayName, profile._json.picture);
			done(null, user);
		}
	}
	catch(e) {
...
const facebookAuth = async (req, accessToken, refreshToken, profile, done) => {
	try {
		if(req.user) {
			...
			const user = await User.oAuthLogin(profile.provider, profile.id, profile.emails[0].value, profile.displayName, profile.photos[0].value);
			done(null, user);
		}
	}
	catch(e) {
